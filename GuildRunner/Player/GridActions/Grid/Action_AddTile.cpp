...
void AAction_AddTile::ExecuteGridAction(FIntPoint TileIndex)
{
	...
	if (!Valid)
	{
		...
		NewTileTransform.SetLocation(
			PlayerGridActions->GetCombatGridReference()->GetTileLocationFromGridIndex(TileIndex));
		...
		NewTileTransform.SetRotation(
			PlayerGridActions->GetCombatGridReference()->GetTileRotationFromGridIndex(TileIndex).Quaternion());
