...
const FCombatGridUnitData* UUnitsUtilities::GetDefaultUnitDataFromUnitType(EUnitType UnitType)
{
	...
	if (!DataTable)
	{
		UE_LOG(LogTemp, Error,
		       TEXT(
			       "[UUnitsUtilities::GetDefaultUnitDataFromUnitType]:\tError: no unit data table assigned to this class, cannot spawn grid."
		       ));
		return nullptr;
	}
	...
	UE_LOG(LogTemp, Error,
	       TEXT("[UUnitsUtilities::GetDefaultUnitDataFromUnitType]:\t Could not dereference row information"));
	return nullptr;
}
...
{
	static ConstructorHelpers::FObjectFinder<UDataTable> GridShapeObject(TEXT(
		"/Script/Engine.DataTable'/Game/GuildRunner/Units/Utilities/DT_DefaultUnitDataPerType.DT_DefaultUnitDataPerType'"));
